...
var checkPings = function(){
  //find users with pings ready to fire
  var userQuery = User.find({'pings.fireAt': {$lt: Date.now()}}, function(err){
    ...
    users.forEach(function(user){
      ...
      user.pings.forEach(function(ping){
        ...
        if (ping.fireAt < Date.now()){
          ...
          firePing(ping, user).then(function(){
            if(ping.recurring){
...
var firePing = function(ping, user){
  return new Promise(
